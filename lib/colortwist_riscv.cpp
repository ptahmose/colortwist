    ...
    const float* twistMatrix)
{
    ...
    const vfloat32m2_t vmin = __riscv_vfmv_v_f_f32m2(0.0f, 1);
    const vfloat32m2_t vmax = __riscv_vfmv_v_f_f32m2(65535.0f, 1);

    const std::ptrdiff_t elemStrideBytes = 3 * static_cast<std::ptrdiff_t>(sizeof(std::uint16_t));
    ...
    for (uint32_t y = 0; y < height; ++y)
    {
        ...
        while (x < width)
        {
            ...
            vuint16m1_t r16 = __riscv_vlse16_v_u16m1(base + 0, elemStrideBytes, vl);
            vuint16m1_t g16 = __riscv_vlse16_v_u16m1(base + 1, elemStrideBytes, vl);
            vuint16m1_t b16 = __riscv_vlse16_v_u16m1(base + 2, elemStrideBytes, vl);

            // Widen + convert to float (use LMUL=2 to keep capacity reasonable)
            ...
            // rDst = b0 + m00*vr + m01*vg + m02*vb   (FMACC chain)
            ...
            // Clamp to [0..65535]
            rDst = __riscv_vfmax_vv_f32m2(rDst, __riscv_vfmv_v_f_f32m2(0.0f, vl), vl);
            rDst = __riscv_vfmin_vv_f32m2(rDst, __riscv_vfmv_v_f_f32m2(65535.0f, vl), vl);

            gDst = __riscv_vfmax_vv_f32m2(gDst, __riscv_vfmv_v_f_f32m2(0.0f, vl), vl);
            gDst = __riscv_vfmin_vv_f32m2(gDst, __riscv_vfmv_v_f_f32m2(65535.0f, vl), vl);

            bDst = __riscv_vfmax_vv_f32m2(bDst, __riscv_vfmv_v_f_f32m2(0.0f, vl), vl);
            bDst = __riscv_vfmin_vv_f32m2(bDst, __riscv_vfmv_v_f_f32m2(65535.0f, vl), vl);

            // Convert back to unsigned ints, then narrow to u16
            ...
            // We already clamped, so vnclipu shift=0 is fine.
            ...
            __riscv_vsse16_v_u16m1(outBase + 0, elemStrideBytes, ro16, vl);
            __riscv_vsse16_v_u16m1(outBase + 1, elemStrideBytes, go16, vl);
            __riscv_vsse16_v_u16m1(outBase + 2, elemStrideBytes, bo16, vl);
